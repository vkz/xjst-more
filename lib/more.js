...
Compiler.prototype.translate = function translate(ast, code, previous) {
  ...
  var result = xjst.translate(ast, code, this.options);
  return { ast: this.replaceContext(result.ast),
           ir: result.ir };
};
...
// *server*
// invoked: pretranslate -> flatten
// wrap templates into template() calls
// replace 'applyCtx' with 'applyNext'
...
Compiler.prototype.generate = function generate(code) {
  ...
  // why convert Mozilla ast to Uglify at all? Wouldn't Escodegen be
  // less costly?  And we could preserve oldTemplates as property on
  // the AST itself (see 'base.js' in xjst).
...
function Server(options) {
  this.bemxjst = new Compiler(options);
  this.options = this.bemxjst.options;
};
exports.Server = Server;

Server.prototype.freshCompiler = function freshCompiler() {
  return new Compiler(this.options);
};

Server.prototype.generate = function generate(code) {
  return (new Compiler(this.options)).generate(code);
};

Server.prototype.compile = function compile(code) {
  return (new Compiler(this.options)).compile(code);
};

Server.prototype.sendGenerate = function sendGenerate(code, client) {
  var ast = esprima.parse(code, {
    loc: true
  });
  var sendTo = this.translateMoreAndSend(ast, code);
  return sendTo(client);
};

Server.prototype.sendCompile = function sendCompile(code, client) {
  var result = this.sendGenerate(code, client),
      exports = {};
  vm.runInNewContext(result.out, { exports: exports, console: console });
  exports.ir = result.ir;
  return exports;
...
Server.prototype.translateMoreAndSend = function translateMoreAndSend(ast, code) {
  ast = this.bemxjst.pretranslate(ast);
  var message = this.prepareTemplatesForSending(ast, code); //need xjst options?
  return function (client) {
    var templates = message.templates;
    var compiler = message.compiler;
    return client.generate(templates, compiler);
  };
};

Server.prototype.prepareTemplatesForSending = function prepareTemplatesForSending(ast, code) {
  var result = this.xjstGenerate(ast, code); // { out: html, ir: [Template, ...] }
  var templates = this.marshalTemplates(result.ir);
  var compiler = this.marshalCompiler(result.compiler);
  return this.message = {templates: templates, compiler: compiler};
};

// internal to server (taps xjst)
Server.prototype.xjstGenerate = function xjstGenerate(ast, code) {
  var xjstCompiler = new xjst.Compiler(this.options);
  (this.result = xjstCompiler.translate(ast, code)).compiler = xjstCompiler;
  return this.result;
};
...
function Client(options, state) {
  this.bemxjstCompiler = new Compiler(options);
  this.xjstOptions = this.bemxjstCompiler.options;
  if (state) {
    ...
    this.templates = [];
    this.html = '';
    this.compiler = new xjst.Compiler(this.xjstOptions);
    this.compiler.empty = true;
    this.compiler.code = '';
  }
}
exports.Client = Client;

Client.prototype.generate = function generate(templates, compiler) {
  compiler = this.unmarshal(compiler);
  templates = this.unmarshalTemplates(templates);
  var xjstResult = xjst.translateMore(this.templates, this.compiler, templates, compiler);
  // -> {ast: , ir: , compiler: }
  var bemxjstResult = {
    ast: this.bemxjstCompiler.replaceContext(xjstResult.ast),
    ir: xjstResult.ir
  };
  this.compiler = xjstResult.compiler;
  this.templates = xjstResult.ir;
  return {
    out: uglify.AST_Node.from_mozilla_ast(bemxjstResult.ast).print_to_string({ beautify: true }),
    ir: bemxjstResult.ir
  }
};

Client.prototype.unmarshal = function unmarshal(compilerMetaData) {
  return JSON.parse(compilerMetaData);
};

Client.prototype.unmarshalTemplates = function unmarshalTemplates(templates) {
  var entities = require('./');
  var Template = entities.Template;
  var Predicate = entities.Predicate;
  var Body = entities.Body;
  var compiler = new xjst.Compiler();

  templates = JSON.parse(templates);

  var result = templates.map(function (t, index) {
    var predicates = t.predicates.map(function (predicate) {
      var newPredicate = new Predicate(compiler, predicate.expr, predicate.value, true);
      newPredicate.id = predicate.id
      newPredicate.valueId = predicate.valueId;
      return newPredicate;
    });

    var body = new Body(compiler, t.body, undefined, undefined, true);
    var predicate = body.applyNext && body.applyNext.pred;
    if (predicate) {
      var newPredicate = body.applyNext.pred = new Predicate(compiler, predicate.expr, predicate.value, true);
      newPredicate.id = predicate.id
      newPredicate.valueId = predicate.valueId;
      assert(newPredicate == body.applyNext.pred);
    }
