...
Server.prototype.freshCompilers = function freshCompilers() {
  this.result = {};
  this.bemCompiler = new Compiler(this.options);
  this.xjstCompiler = new xjst.Compiler(this.options);
  return this.xjstCompiler;
};

Server.prototype.generate = function generate(code, client) {
  this.freshCompilers();
  var ast = esprima.parse(code, {
    loc: true
  });
  var sendTo = this.translate(ast, code);
  return sendTo(client);
};

Server.prototype.compile = function compile(code, client) {
  var result = this.generate(code, client),
      exports = {};
  vm.runInNewContext(result.out, { exports: exports, console: console });
  exports.ir = result.ir;
  return exports;
...
Server.prototype.translate = function translate(ast, code) {
  ast = this.bemCompiler.pretranslate(ast);
  var result = this.result = this.xjstCompiler.translate(ast, code); // { out: html, ir: [Template, ...] }
  var message = this.message = {
    templates: this.marshalTemplates(this.result.ir),
    compiler: this.marshalCompiler(this.xjstCompiler)
  };
  return function (client) {
    return client.generate(message.templates, message.compiler);
  };
};
