...
function Client(options, state) {
  this.templates = [];
  this.code = '';
  this.bemCompiler = new Compiler(options);
  this.xjstCompiler = new xjst.Compiler(options);
  this.xjstCompiler.empty = true;
...
Client.prototype.translate = function translate(newTemplates, newCompiler) {
  var compiler = this.xjstCompiler,
      idMap = compiler.merge(newCompiler);
  compiler.switchCompilerInTemplates(newTemplates, idMap);
  var result = compiler.translateTemplates(newTemplates, this.templates);
  this.xjstCompiler = compiler;
  return result;
}
...
Client.prototype.unmarshalTemplates = function unmarshalTemplates(data) {
  var constructors = require('./'),
      ...
      compiler = new xjst.Compiler(),
      templates = JSON.parse(data),
      CLONE = true;

  function wrapPredicate(predicate) {
    var result = new Predicate(compiler, predicate.expr, predicate.value, CLONE);
  ...
  return templates.map(function (template) {
    var predicates = template
        ...
        body = new Body(compiler, template.body, undefined, undefined, CLONE),
