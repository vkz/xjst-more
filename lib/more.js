...
Compiler.prototype.translateMoreAndSend = function translateMoreAndSend(ast, code) {
  ...
  var send = xjst.prepareTemplatesForSending(ast, code, self.options);
  return function (client) {
    var xjstResult = send(client);
    // TODO replaceContext should be happening on the client. Do we also need to
    // merge bem-xjst compiler used on the server for this to work?
    var bemxjstResult = {
      ast: client.bemxjstCompiler.replaceContext(xjstResult.ast),
      ir: xjstResult.ir
    };
    return {
