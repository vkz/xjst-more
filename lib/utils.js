...
// clone pre-rollOut-sortGroup templates
utils.cloneTemplates = function cloneTemplates(ast) {
  if (ast === null || ast === undefined ||
      typeof ast === 'number' || typeof ast === 'string' ||
      typeof ast === 'boolean' || ast instanceof RegExp) {
    return ast;
  }
  if (Array.isArray(ast)) return ast.map(cloneTemplates);

  var res = {};
  Object.keys(ast).forEach(function(key) {
    if(key !== 'compiler'){
      res[key] = cloneTemplates(ast[key]);
    }
  })
...
utils.replaceContext = function replaceContext(ast) {
  debugger;
  var globals = {
    _mode: '$$mode',
    block: '$$block',
