...
Compiler.prototype._translate2 = function translate2(program, bodyOnly, ir) {
  ...
  program.templates.forEach(function (template) {
    var body = {type: "Program", body: template.body.body};
    template.body.$body = esgen(body);
    template.predicates = template.predicates.map(function (p) {
      var lhs = { type: "Program", body: p.expr },
          // value can be null
          rhs = p.value && { type: "Program", body: p.value };
      p.$expr = esgen(lhs);
      p.$value = rhs && esgen(rhs);
      return p;
    });
  }, this);
  ...
  // TODO merge $resetApplyNext, $recordExtensions, $initializers with client
  // versions
