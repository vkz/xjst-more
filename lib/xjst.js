...
exports.translate = function translate(ast, code, options, previous) {
  ...
  if (previous) {
    ...
    DEBUG && console.log("Client xjst compiler\n", "--------------------\n", compiler);

    var fromServer = serverGenerate(ast, code, options);
    var serverCompiler = fromServer.ir[0].compiler;
    var templates = fromServer.ir;

    DEBUG && console.log("Server xjst compiler\n", "--------------------\n", serverCompiler);

    compiler.merge(serverCompiler);

    DEBUG && console.log("Merged xjst compiler\n", "--------------------\n", compiler);

    compiler.switchCompilerInTemplates(templates);
...
Compiler.prototype.merge = function merge(compiler) {
  ...
  Object.keys(idHash).forEach(function (key) {
    ...
    revIdHash[idHash[key].id] = this.idHash[key];
  }, this);
...
Compiler.prototype.switchCompilerInTemplates = function switchCompilerInTemplates(templates) {
  ...
  templates.forEach(function (template) {
    ...
    template.predicates.forEach(function(predicate) {
      var oldCompiler = predicate.compiler;
      var oldId = predicate.id;
      var oldValueId = predicate.valueId;
      // update to id set in merge()
      predicate.id = oldCompiler.revIdHash[oldId].id;
      // update to valueId set in merge()
      predicate.valueId = oldCompiler.revIdHash[oldValueId].id;
