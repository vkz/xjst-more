...
// TODO separate execution into two paths
// (1)
// -- translateMore(templates, compiler, previousTemplates, previousCompiler)
// -- compiler.translateTemplates(templates, previousTemplates)
//
// NOTE be careful with `compiler' instances. Looks like `this' refers to
// previousCompiler already, so maybe no need to pass previousCompiler.
//
// (2)
// -- translate(ast, code, options)
// -- compiler.translate(ast, code)

// TODO Server-side
exports.prepareTemplatesForSending = function prepareTemplatesForSending(ast, code, options) {
  var result = serverGenerate(ast, code, options); // { out: html, ir: [Template, ...] }
  // TODO marshal templates
  var templates = marshalTemplates(result.ir);
  var compiler = marshal(result.compiler);
  return {templates: templates, compiler: compiler};
}

function marshalTemplates(templates) {
  // console.log("!!!templates to marshal = ", templates);
  var json = JSON.stringify(templates.map(function (t) { return t.marshal(); }));
  return json;
}

exports.translateMore = function translateMore(oldTemplates, oldCompiler,
                                               newTemplates, newCompiler) {
    var idMap = oldCompiler.merge(newCompiler);
    assert(oldCompiler == oldTemplates[0].compiler);
    oldCompiler.switchCompilerInTemplates(newTemplates, idMap);
    return oldCompiler.translateTemplates(newTemplates, oldTemplates);
}
...
// TODO marshal for templates
// TODO unmarshal for templates

function marshal(compiler) {
  var json = {
    code : compiler.code,
  ...
  var DEBUG = false;
  DEBUG && pp(json, {prompt: "initial"});
  DEBUG && pp(unmarshal(string), {prompt: "unmarshaled"});

  return string;
};

function serverGenerate(ast, code, options) {
...
// TODO make it functional. Create and return a new merged compiler instead of
// mutating the existing one.
Compiler.prototype.merge = function merge(compiler) {
  // idMap maps old id|valueId into new id|valueId
  var idMap = {};
  var code = compiler.code,
      ...
      init = compiler.inputProgram.init,
      other = compiler.inputProgram.other,
      oldInit = this.inputProgram.init,
      oldOther = this.inputProgram.other;

  this.code += code || '';

  // TODO: looks like this.inputProgram is an altefact of older
  // inliner and is no longer used. Get init and other out of
  // this.program instead and ditch inputProgram.
  oldInit = oldInit.concat(init);
  oldOther = oldOther.concat(other);

  // merge idHash and revIdHash
  // increment idCount by the number of new keys in idHash
  Object.keys(idHash).forEach(function (key) {
    var id;
    if (this.idHash.hasOwnProperty(key)) {
      id = this.idHash[key].id;
      this.idHash[key].score += idHash[key].score;

      // TODO: do I actually need to reassign this ref? Don't think
      // so. Try it both ways.
      this.revIdHash[id] = this.idHash[key];
    } else {
      id = this.idCount++;
      this.idHash[key] = { id: id, key: key, value: idHash[key].value, score: idHash[key].score };
      this.revIdHash[id] = this.idHash[key];
    }
    // TODO: this is so I can fix id and valueId in any Predicate
    // in new Templates. Don't forget to do this when replacing new
    // compiler refs with merged compiler in the new Templates. This
    // assignment should allow me to lookup correct hash objects by
    // using ids and valueIds in Predicates for new Templates
    // (reverse lookup) in revIdHash of the new compiler.
    idMap[idHash[key].id] = this.idHash[key];
  }, this);
  ...
  Object.keys(scores).forEach(function (key) {
    if (this.scores.hasOwnProperty(key)) {
      ...
      // copy over values that weren't in old templates incrementing the variance
      Object.keys(newitem.values).forEach(function (value) {
        if (!item.values.hasOwnProperty(value)) {
          item.values[value] = newitem.values[value];
          item.variance++;
        } else {
          item.values[value] += newitem.values[value];
        }
...
Compiler.prototype.switchCompilerInTemplates = function switchCompilerInTemplates(templates, idMap) {

  // TODO: need to bump Entity ids to account for existence of older
  // templates. For example:
  //
  // - any uid in Body instance needs to increase by this.bodyUid.
  //
  // - any uid in Template instance (which inherits uid from
  // GenericBody) needs to increase by this.bodyUid
  //
  // - any Predicate needs to update id and valueId by looking them up
  // in revIdHash of new compiler (this === Predicate instance):
  // this.id = this.compiler.revIdHash[this.id].id
  // this.valueId = this.compiler.revIdHash[this.valueId].id


  // NOTE: idMap maps old ids and valueIds into their new ids for
  // predicates that are in new templates but weren't in the old ones
  templates.forEach(function (template) {
    // update predicates
    template.predicates.forEach(function(predicate) {
      var oldId = predicate.id;
      var oldValueId = predicate.valueId;
      // update to id set in merge()
      predicate.id = idMap[oldId].id;
      // update to valueId set in merge()
      predicate.valueId = idMap[oldValueId].id;
      predicate.compiler = this;
    }, this);
    template.uid += this.bodyUid;
    template.compiler = this;
    template.body.uid += this.bodyUid;
    template.body.compiler = this;
    // TODO: template.body.id = ??? is tricky. Seems that it's only assigned by
    // calling shareBody() when rendering and cloning.
  }, this);
};
