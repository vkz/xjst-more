...
Compiler.prototype._translate2 = function translate2(program, bodyOnly, ir) {
  ...
  program.templates.forEach(function(template) {
    ...
    // var __temp = esgen(utils.replaceContext(__body));
    // var __ctx = /__\$ctx\.(_mode|block|elem|elemMods|mods)/g.test(__temp) && '__ctx';
    // var $$ctx = /\$\$mode|\$\$block|\$\$elem|\$\$elemMods|\$\$mods/g.test(__temp) && '$$ctx';
    // if(__ctx || $$ctx) { 
    //   pp(__temp, {prompt: "replaceContext [ " + (__ctx || '') + ($$ctx || '') + " ]"});
    // };
    ...
    // DEBUG body that definitely gets inlined
    if(/__\$ctx\.(_mode|block|elem|elemMods|mods)/g.test(__body.asString) &&
       /BEM_INTERNAL__\$140/g.test(__body.asString)) { 
      //pp(__bodyInlined.asString, {prompt: "__bodyInlined as string"});
      //pp(esgen(utils.replaceContext(__body)), {prompt: "replaced context"});

      var __Comp = require('./more').Compiler;
      var __comp = new __Comp();
      var __ast = __comp.replaceContext({
        type: "Program",
        body: [{ 
          type: 'FunctionDeclaration',
          id: { type: 'Identifier', name: '__f' },
          params: [],
          defaults: [],
          body: 
          { type: 'BlockStatement',
      ...
      pp(esgen(__ast), {prompt: "replaced context"});
