...
exports.translate = function translate(ast, code, options, previous) {
  ...
  var oldCode;
  if (previous) {
    compiler = previous.ir[0].compiler;
    oldCode = compiler.code;

    var fromServer = serverGenerate(ast, code, options);
    var serverCompiler = fromServer.ir[0].compiler;
    var templates = fromServer.ir;

    // TODO: is there any compeling reason to separate compiler merge
    // and switching compiler in templates? Without this separation I
    // can avoid passing idMap around.
    var idMap = compiler.merge(unmarshal(marshal(serverCompiler)));
    // compiler.merge(serverCompiler);
    assert(compiler == previous.ir[0].compiler);

    compiler.switchCompilerInTemplates(templates, idMap);

    return compiler.translateTemplates(templates, previous.ir);
    
  } else {
    compiler = create(options);
    return compiler.translate(ast, code, previous, oldCode);
  }
};

// TODO: marshal(server) compiler metadata as JSON
