...
function Client(state) {
  if (state) {
    ...
    this.html = state.out;
    this.compiler = state.ir[0].compiler;
  } else {
    throw new Error('state parameter cannot be nil')
  }
}
exports.Client = Client;

exports.sendTemplates = function sendTemplates(ast, code, options) {
  var result = serverGenerate(ast, code, options); // { out: html, ir: [Template, ...] }
  var templates = result.ir;
  var compiler = result.compiler;
  return function (client) {
    return exports.translateMore(client.templates, client.compiler, templates, compiler);
  };
}

exports.translateMore = function translateMore(oldTemplates, oldCompiler,
                                               newTemplates, newCompiler) {
    var idMap = oldCompiler.merge(unmarshal(marshal(newCompiler)));
...
function unmarshal(compilerMetaData) {
  return JSON.parse(compilerMetaData);
};
