...
Compiler.prototype._translate2 = function translate2(program, bodyOnly) {
  ...
  if (globalIR) {
    program.templates = program.templates.concat(globalIR);
  }
  globalIR = program.templates.map(function (template) {return template.clone();});
...
exports.translate = function translate(ast, code, options, ir) {
  ...
  var compiler;
  if (ir) {
    compiler = (globalIR = ir)[0].compiler;
    oldCode = compiler.code;
  } else {
    compiler = create(options);
  }

  return {ast: compiler.translate(ast,  code), ir: globalIR};
};
