...
Compiler.prototype.translateTemplates = function translateTemplates(templates, previousTemplates) {
  ...
  var result = this._translate3(program, false, previousTemplates);
...
Compiler.prototype.translate = function translate(ast, code, previous, oldCode) {
  ...
  // TODO seems like leftover from earlier implementations. Cleanup of this
  // whole inline business is in order.
  this.inlineDepth = this.maxInlineDepth;
  ...
  // NOTE inlining has moved into _translate2 before sortGroup
 // result.ast = this.inliner.inline(result.ast);
...
Compiler.prototype._translate2 = function translate2(program, bodyOnly, ir) {
  ...
  // TODO merge $resetApplyNext, $recordExtensions, $initializers, $other,
  // $declareGlobals, $initializeGlobals with client versions
...
Compiler.prototype._translate3 = function translate3(program, bodyOnly, ir) {
  var cacheKey = this._renderCacheKey(program.templates);
  var res = this.probeRenderCache(cacheKey);
  if (res && bodyOnly) return xjst.utils.cloneAst(res);
